...
async function uploadFiles(fileListOrArr) {
  ...
  try {
    ...
    if (!data.session_id || !data.uploaded) {
    ...
    alert("Upload failed: " + (msg || "network or server error. Try a smaller image or wait a few seconds and retry."));
  }
}
...
btnAnalyze.addEventListener("click", async () => {
  ...
  // Set types on server (JSON — avoids comma / encoding issues)
  const assignments = state.documents.map((d) => ({ id: d.id, type: d.type }));
  if (assignments.some((a) => !a.id || !a.type)) {
    alert("Please select a document type for every file.");
    return;
  }
  const formTypes = new FormData();
  formTypes.append("session_id", state.sessionId);
  formTypes.append("assignments", JSON.stringify(assignments));
  ...
  try {
    ...
    alert("Analysis error: " + (e.message || "unknown") + "\n\nTip: On the free plan, complete upload → type → analyze without long pauses (the server may sleep and lose the session).");
    showStep("step-types");
  }
});
